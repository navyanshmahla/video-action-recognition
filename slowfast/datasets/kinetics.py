...
from . import random_erasing as random_erasing
    ...
    def _construct_loader(self):
        ...
        with g_pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                # chinu = len(path_label.split(self.cfg.DATA.PATH_LABEL_SEPARATOR))
                # print(f"Len : {chinu}")
        ...
        self.valid_indices = [i for i in range(len(self._path_to_videos))]
        print(f"Total videos: {len(self._path_to_videos)}", flush=True)
        print(f"Valid videos: {len(self.valid_indices)}", flush=True)
