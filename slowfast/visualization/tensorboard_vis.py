...
class TensorboardWriter(object):
    ...
    def __init__(self, cfg):
        ...
        if cfg.TENSORBOARD.CLASS_NAMES_PATH != "":
            ...
            class_names_info = get_class_names(
               cfg.TENSORBOARD.CLASS_NAMES_PATH,
               cfg.TENSORBOARD.CATEGORIES_PATH,
               cfg.TENSORBOARD.CONFUSION_MATRIX.SUBSET_PATH,
            )

            if class_names_info is not None and len(class_names_info) == 3:
                # Unpack the returned tuple safely
                self.class_names, self.parent_map, self.cm_subset_classes = class_names_info
            else:
            # Handle the case where class_names_info is None or not as expected
                print("Error: get_class_names() did not return the expected values.")
                print(f"TENSORBOARD.CLASS_NAME_PATH = {type(cfg.TENSORBOARD.CLASS_NAMES_PATH)}")
                print(f"TENSORBOARD.CATEGORIES_PATH = {type(cfg.TENSORBOARD.CATEGORIES_PATH)}")
                print(f"TENSORBOARD.CONFUSION_MATRIX.SUBSET_PATH = {type(cfg.TENSORBOARD.CONFUSION_MATRIX.SUBSET_PATH)}")
